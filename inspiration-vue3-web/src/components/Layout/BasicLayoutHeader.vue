...
import { User, VideoPause, ArrowDown } from '@element-plus/icons-vue';
...
const isCollectionActive = ref(false)
const ActiveLink = () => {
  isCollectionActive.value = ['Collection', 'CollectionDetail'].includes(route.name);
};
        ...
        <div class="header-content container-1080">
            <div class="header-hd">
                <router-link :to="{ name: 'Home' }">
                    <img class="header-logo" src="@/assets/images/icons/header-logo.svg" />
                </router-link>
            </div>
            <div class="header-ft">
                <nav class="header-nav">
                    <router-link :to="{ name: 'Home' }" class="header-nav-item">首页</router-link>
                    <router-link :to="{ name: 'Collection' }" class="header-nav-item" :class="{ 'router-link-exact-active': isCollectionActive }">收藏夹</router-link>
                </nav>
                ...
                <el-dropdown style="height: 100%" @command="handleCommand" v-if="userInfo?.id">
                    <div class="bar-info-container" @mouseover="isHovered = true" @mouseout="isHovered = false">
                        ...
                        <i class="el-icon-arrow-down el-icon--right" :class="{ 'rotate-up': isHovered }">
                            <el-icon>
                                <ArrowDown />
                            </el-icon>
                        </i>
